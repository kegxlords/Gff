...
const log = (t, m, d) =>
  console.log(
    `[${new Date().toISOString()}] [${t}] ${m}${d ? ' ' + JSON.stringify(redact(d)) : ''}`
  );

const safe200 = (res, msg) =>
  res.status(200).json({
    ok: true,
    msg
  });
...
async function creditDeposit({
  ...
  if (error || !wallet) {
    ...
    if (insertErr) {
      throw new Error(
        'Failed to create wallet: ' + insertErr.message
      );
    }
  ...
  await supabase
    ...
    .update({
      balance: nb,
      total_deposit:
        Number(wallet.total_deposit || 0) + amt,
      updated_at: new Date().toISOString()
    })
    .eq('user_id', user_id);
    ...
    if (rw) {
      ...
      await supabase
        ...
        .update({
          ...
          total_referral_earnings:
            Number(rw.total_referral_earnings || 0) + comm,
          updated_at:
      ...
      await supabase
        ...
        .insert({
          ...
          description:
            pct +
            '% commission on downline deposit [' +
            user_id +
            ']',
          balance_after: rb
        });
...
async function handleDeposit(
  ...
  if (Number(body.status) === 1) {
    ...
    const {
      ...
      description:
        `OTPay deposit (${MODE}) — ref ${body.orderId}`
    });
    ...
    if (tx.linked_request_id) {
      ...
      await supabase
        ...
        .update({
          status: 'approved',
          admin_note:
            `OTPay payout successful (${MODE})`
        })
        .eq(
          'id',
          tx.linked_request_id
        );
    }
    ...
    if (tx.linked_request_id) {
      ...
      const {
        ...
        .update({
          status: 'rejected',
          admin_note:
            `OTPay payout failed — refunded (${body.msg || ''})`
        })
        .eq(
          'id',
          tx.linked_request_id
      ...
      if (
        ...
        if (w) {
          ...
          await supabase
            ...
            .insert({
              ...
              amount: refund,
              description:
                `OTPay payout failed — refund (${MODE})`,
              balance_after: nb
            });
        }
        ...
        log(
          'otpay.withdraw-callback',
          'refund skipped — request already settled',
          {
            requestId:
              tx.linked_request_id
          }
...
module.exports = async function handler(
  ...
  if (!oid) {
    return safe200(
      res,
      'ignored: missing merchantOrderId'
    );
  }
  ...
  const kind =
    existingTx?.linked_request_id
      ? 'payout'
      : 'deposit';

  const eventKey =
    `${kind}:${oid}:${body.status}:${body.timestamp || ''}`;
