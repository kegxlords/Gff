...
module.exports = async function handler(req, res) {
  ...
  if (!allowed && authHeader.startsWith('Bearer ')) {
    ...
    if (data?.user) {
      ...
      if (p?.is_admin) allowed = true;
    }
  }

  if (!allowed && !secret) allowed = true; // dev fallback when no CRON_SECRET set
  ...
  try {
    ...
    for (const p of plans || []) {
      ...
      const { data: wallet } = await supabase.from('wallets').select('*').eq('user_id', p.user_id).single();
      if (!wallet) continue;
      ...
      await supabase.from('wallet_transactions').insert({
        user_id: p.user_id, type: 'daily_income', amount: income,
